...
int	rush(const int width, const int height)
{
	...
	while (current_line <= height)
	{
		...
		while (current_column <= width)
		{
			...
			if (((current_column == 1)&&(current_line == 1))||((current_column == width)&&(current_line == 1))||((current_column == 1)&&(current_line == height))||((current_column == width)&&(current_line == height)))
			{
				ft_putchar('o');
			}
            /*Comprobamos si es lado superior o inferior*/
            else if (((current_line == 1)||(current_line == height))&&((current_column != 1) && (current_column != width)))
			{
                ft_putchar('-');
            }
            /*Comprobamos si lado izquierdo o derecho*/
            else if(((current_column == 1)||(current_column == width))&&((current_line != 1) && (current_line != height)))
			{
                ft_putchar('|');
            }
